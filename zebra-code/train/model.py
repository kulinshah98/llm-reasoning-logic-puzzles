...
class TransformerLMHeadModel(nn.Module):
    ...
    def __call__(self, inputs, training=True):
        ...
        token_embed_with_init = functools.partial(
            nn.Embed, embedding_init=nn.initializers.normal(stddev=0.02))
        token_embeddings = token_embed_with_init(
        ...
        pos = jnp.arange(0, seq_size, dtype=jnp.int32)[None]
        pos_embed_with_init = functools.partial(
            nn.Embed, embedding_init=nn.initializers.normal(stddev=0.02)
        )
        pos_embeddings = pos_embed_with_init(
            num_embeddings=self.config.max_seq_len,
            features=self.config.emb_dim
        )(pos)
        
        output_tuple = (pos_embeddings.shape, token_embeddings.shape[1:], seq_size)
        assert pos_embeddings.shape[1:] == token_embeddings.shape[1:], output_tuple
